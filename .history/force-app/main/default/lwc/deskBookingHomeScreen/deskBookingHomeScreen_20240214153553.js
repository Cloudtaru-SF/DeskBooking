...
export default class DeskBookingHomeScreen extends LightningElement {
    ...
    inputsHandler(event){
        ...
        if(event.target.name == 'buildingName'){
            ...
            let buildingname = this.floorOptions.find(floor => floor.value = this.buildingName).label;
            console.log(`/${buildingname}${this.floorNumber}.jpg`);
            let s = `/${buildingname}${this.floorNumber}.jpg`;
            this.floorImage = DeskBooking + '/Awfis1';
        }
        if(event.target.name == 'floorNumber'){
            this.floorNumber = event.target.value;
            let buildingname = this.floorOptions.filter(ele=>ele.value==this.buildingName)[0].label;
            console.log(`/${buildingname}${this.floorNumber}.jpg`);
            let s = `/${buildingname}${this.floorNumber}.jpg`;
    ...
    getBuildingAndFloors(){
        ...
        .then(result=>{
            ...
            this.floorNumber = '1';
            let s = `/${this.buildingName}${this.floorNumber}.jpg`;
            this.floorImage = DeskBooking + '/Awfis1';
