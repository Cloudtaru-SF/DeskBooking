...
import updateWorkspaceRecords from '@salesforce/apex/SeatBookingHelper.updateWorkspaceRecords';


import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import homeIcon from '@salesforce/resourceUrl/Homeicon';
import profileIcon from '@salesforce/resourceUrl/ProfilePictureIcon';
...
export default class DeskBookingHomeScreen extends LightningElement {
    ...
    @track showBookButton = false;
    @track approveRejectModal = false;
    @track noDataImage = DeskBooking+'/5484597.jpg';
    iconHome = homeIcon;
    profilePicIcon = profileIcon;
    @track logoutModal = false;
    ...
    selectedIdsHandler(event){
        ...
        this.workspaceNo = '';
        if(this.bookedForDate >= new Date().toISOString().substring(0, 10)){
            this.showBookButton = this.selectedIds.length>0 && this.empRole != 'Admin' ? true : false;
        }
    ...
    saveHandler(event){
        ...
        records = this.selectedIds.map(ele=>{
                                        return {"BookedforDate__c":this.bookedForDate,
                                                "BookingStatus__c": this.empRole == 'Manager' ? "Booked" : "Pending",
    ...
    approveRejectModalAction(event){
        this.deleteModal = true;
        if(event.currentTarget.dataset.role == 'Employee' && ['Pending'].includes(event.currentTarget.dataset.status)){
    ...
    approveRejectAction(event){
        let records = [];
        let record = {"Id":event.currentTarget.dataset.id,
                       "BookingStatus__c": event.currentTarget.dataset.value};
        records.push(record);
        updateWorkspaceRecords({records})
        .then(result=>{
            if(result == 'success'){
                this.deleteModal = false;
                this.showModal = false;
